let guestList: string[] = ["Albert Einstein", "Marie Curie", "Neil Armstrong", "Isaac Newton", "Ada Lovelace"];
...
for (let i: number = 0; i < guestList.length; i++) {
  console.log(`Dear ${guestList[i]}, you are still cordially invited to dinner at my place!`);
}

// Print a message to each removed guest
for (let i: number = 0; i < removedGuests.length; i++) {
  console.log(`Dear ${removedGuests[i]}, I'm sorry to inform you that my new dinner table won't arrive in time, so I won't be able to host you this time. I hope we can meet soon!`);
}
